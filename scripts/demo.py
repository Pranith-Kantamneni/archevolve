...
def demo_basic() -> None:
    ...
    # 4-6. EVALUATION, RANKING, SELECTION (via EvolutionEngine & Multi-Agent Orchestrator)
    print_section("4. MULTI-AGENT EVOLUTIONARY PROCESS")
    print_line("Orchestrator", "Active Multi-Agent Workflow")
    print_line("Participating Agents", "Requirement, Generation, 5x Evaluators, Selection, Evolution, Experience Memory, Final Architecture")
    ...
    print_line("Initial candidates", "5 architectures generated by Generation Agent")
    print_line("Selection count", f"{engine.selection_count} candidates selected per generation")
    print_line("Mutation count", f"{engine.mutation_count} mutations produced per selected candidate")

    # 7. MUTATION / EVOLUTION
    print_section("7. ARCHITECTURE EVOLUTION / MUTATION AGENT")
    ...
    if improved_obj:
        ...
        print_line("  Objectives improved", "none")

    # 8. AGENT EVOLUTION TRACE HIGHLIGHTS
    print_section("8. AGENT EVOLUTION TRACE (Sample)")
    traces = results.get("agent_evolution_trace", [])
    for t in traces[:8]:
        fit_str = f" [fitness={t['fitness']:.1f}]" if t.get("fitness") is not None else ""
        print_line(f"  [{t['agent']}]", f"{t['action']} -> {t['outcome'].upper()}{fit_str}")
        print_line("    Reason", t.get("reason", "")[:95] + ("..." if len(t.get("reason", "")) > 95 else ""))

    # 9. EXPERIENCE MEMORY (SUCCESSES & FAILURES)
    print_section("9. EXPERIENCE MEMORY (SUCCESSES & FAILURES)")
    mem_entries = results["experience_memory_entries"]
    print_line("Total stored experiences", str(mem_entries))
    exp_records = engine.experience_memory.recent_entries(limit=6)
    successes = [e for e in exp_records if e.outcome == "success"]
    failures = [e for e in exp_records if e.outcome == "failure"]
    print_line("Recent Successes", f"{len(successes)} entries")
    for s in successes[:2]:
        print_line("  ✓ SUCCESS", f"{s.architecture_name} (Fit {s.fitness:.1f}): {s.reason}")
    print_line("Recent Failures", f"{len(failures)} entries")
    for f in failures[:2]:
        print_line("  ✗ FAILURE", f"{f.architecture_name} (Fit {f.fitness:.1f}): {f.reason}")

    # 10. FINAL ARCHITECTURE AGENT
    print_section("10. FINAL ARCHITECTURE AGENT")
    final_arch = results["final_architecture"]
    print_line("Winning architecture", final_arch["name"])
    print_line("Final fitness score", f"{results['final_scores']['overall']:.1f}")
    ...
    total_candidates = results["evolution_history"]["candidates_evaluated"]
